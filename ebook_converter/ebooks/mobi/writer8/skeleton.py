...
from ebook_converter.constants import XHTML_NS
from ebook_converter.ebooks.oeb.base import extract
from ebook_converter.ebooks.mobi.utils import to_base, PolyglotDict
...
def tostring(raw, **kwargs):
    ...
    ans = etree.tostring(raw, **kwargs).decode('utf8')
    if xml_declaration:
        ans = '<?xml version="1.0" encoding="%s"?>\n'%encoding + ans
    return re.sub(r'&#x([0-9A-Fa-f]+);', lambda m:str(int(m.group(1), 16)),
            ans).encode(encoding)
...
class Chunker(object):
    ...
    def set_internal_links(self, text, rebuilt_text):
        ...
        placeholder_map = {k.encode('utf-8'):to_placeholder(v) for k, v in
