...
from ebook_converter.ebooks.oeb.base import OPF, OEB_DOCS, XPath, XLINK, xml2text
from ebook_converter.ebooks.oeb.polish.replace import replace_links, get_recommended_folders
...
def create_epub_cover(container, cover_path, existing_image, options=None):
    ...
    if no_svg:
        ...
        if callable(cover_path):
            ...
            templ = templ.replace('__width__', width)
            templ = templ.replace('__height__', height)
    ...
    if ver.major < 3:
        ...
        container.insert_into_xml(guide, guide.makeelement(
            OPF('reference'), type='cover', title=('Cover'),
